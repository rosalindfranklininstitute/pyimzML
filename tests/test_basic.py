...
ALL_TEST_CASES = [
    (parse_lib, data_name, imzml_path, ibd_path)
...
class ImzMLParser(unittest.TestCase):
    def test_bisect(self):
        mzs = [100.0, 201.89, 201.99, 202.0, 202.01, 202.10000001, 400.0]
        test_mz = 202.0
        test_tol = 0.1
    ...
    def test_getspectrum(self):
        for parse_lib, data_name, imzml_path, ibd_path in ALL_TEST_CASES:
            with (
                self.subTest(parse_lib=parse_lib, data=data_name),
    ...
    def test_files_instead_of_paths(self):
        for parse_lib, data_name, imzml_path, ibd_path in ALL_TEST_CASES:
            with (
                ...
                imzmlp.ImzMLParser(
                    imzml_file, parse_lib=parse_lib, ibd_file=ibd_file
                ) as parser,
            ):
                mzs, ints = parser.getspectrum(4)
    ...
    def test_parse_metadata(self):
        for parse_lib, data_name, imzml_path, ibd_path in ALL_TEST_CASES:
            with (
                ...
                assert (
                    md.file_description.source_files["sf1"]["Thermo RAW format"] == True
                )
                assert (
                    md.file_description.source_files["sf1"].attrs["name"]
                    == "Example.raw"
                )
                ...
                assert (
                    md.data_processings["XcaliburProcessing"]
                    .methods[0]
                    .attrs["softwareRef"]
                    == "Xcalibur"
                )
    ...
    def test_parse_full_spectrum_metadata(self):
        for parse_lib, data_name, imzml_path, ibd_path in ALL_TEST_CASES:
            with (
                self.subTest(parse_lib=parse_lib, data=data_name),
                imzmlp.ImzMLParser(
                    imzml_path, parse_lib=parse_lib, include_spectra_metadata="full"
                ) as parser,
            ):
                ...
                assert (
                    spectrum.scans[0].attrs["instrumentConfigurationRef"]
                    == "LTQFTUltra0"
                )
    ...
    def test_parse_partial_spectrum_metadata(self):
        ...
        for parse_lib, data_name, imzml_path, ibd_path in ALL_TEST_CASES:
            with (
                self.subTest(parse_lib=parse_lib, data=data_name),
                imzmlp.ImzMLParser(
                    imzml_path, parse_lib=parse_lib, include_spectra_metadata=ACCESSIONS
                ) as parser,
            ):
    ...
    def test_read_file(self):
        ...
        for parse_lib, data_name, imzml_path, ibd_path in ALL_TEST_CASES:
            with (
                ...
                detached_parser = imzmlp.ImzMLParser(
                    imzml_path, parse_lib=parse_lib, ibd_file=None
                )
                ...
                portable_mzs, portable_ints = portable_reader.read_spectrum_from_file(
                    ibd_file, spectrum_idx
                )
