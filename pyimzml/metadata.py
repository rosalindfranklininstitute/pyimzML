...
from pyimzml.ontology.ontology import (
    lookup_and_convert_cv_param,
    convert_xml_value,
    convert_term_name,
)
...
class _ParseUtils:
    ...
    def param_groups_list(self, parent_node, xpath):
        return [
            self.param_group(n) for n in parent_node.findall(xpath.format(XMLNS_PREFIX))
        ]

    def refs_list(self, parent_node, xpath):
        return [
            n.attrib.get("ref") for n in parent_node.findall(xpath.format(XMLNS_PREFIX))
        ]
...
class Metadata:
    def __init__(self, root):
        ...
        self.file_description = pu.param_group(
            ...
            source_files=pu.param_groups_by_id(
                fd_node, "{0}sourceFileList/{0}sourceFile"
            ),
            contacts=pu.param_groups_list(fd_node, "{0}contact"),
        )

        self.referenceable_param_groups = pu.param_groups_by_id(
            root, "{0}referenceableParamGroupList/{0}referenceableParamGroup"
        )
        ...
        for node in root.findall(
            "{0}scanSettingsList/{0}scanSettings".format(XMLNS_PREFIX)
        ):
            self.scan_settings[node.get("id")] = pu.param_group(
                node,
                source_file_refs=pu.refs_list(
                    node, "{0}sourceFileRefList/{0}sourceFileRef"
                ),
        ...
        for node in root.findall(
            "{0}instrumentConfigurationList/{0}instrumentConfiguration".format(
                XMLNS_PREFIX
            )
        ):
        ...
        for node in root.findall(
            "{0}dataProcessingList/{0}dataProcessing".format(XMLNS_PREFIX)
        ):
            self.data_processings[node.get("id")] = pu.param_group(
                node, methods=pu.param_groups_list(node, "{0}processingMethod")
            )
...
class ParamGroup:
    ...
    def __init__(self, elem, **extra_data):
        ...
        self.param_group_refs = [
            ref.get("ref")
            for ref in elem.findall(
                "{0}referenceableParamGroupRef".format(XMLNS_PREFIX)
            )
        ]
        ...
        self.param_by_accession = {param[1]: param[2] for param in self.cv_params}
    ...
    def __init__(self, root, referenceable_param_groups):
        ...
        for node in root.findall("{0}scanList/{0}scan".format(XMLNS_PREFIX)):
            scans.append(
                pu.param_group(
                    node,
                    scan_windows=pu.param_groups_list(
                        node, "{0}scanWindowList/{0}scanWindow"
                    ),
                )
            )
        ...
        for node in root.findall("{0}precursorList/{0}precursor".format(XMLNS_PREFIX)):
            precursors.append(
                pu.param_group(
                    node,
                    isolation_window=pu.optional_param_group(
                        node, "{0}isolationWindow"
                    ),
                    selected_ions=pu.param_groups_list(
                        node, "{0}selectedIonList/{0}selectedIon"
                    ),
        ...
        binary_data_arrays = pu.param_groups_list(
            root, "{0}binaryDataArrayList/{0}binaryDataArray"
        )
